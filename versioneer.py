...
# Lazily import pkg_resources.parse_version to avoid import-time dependency on setuptools
# This ensures versioneer can be imported in isolated build environments that may
# not have pkg_resources available at module import time.
def parse_version(v):
    try:
        ...
        # Fallback: try packaging.version.parse if available
        try:
            from packaging.version import parse as _parse

            return _parse(v)
        except Exception:
            # Final fallback: return the raw value
            return v
...
def render_pep440_ricequant(pieces):
    ...
    if tag:
        ...
        if parsed_working < parsed_tag:
            if parsed_working.base_version == parsed_tag.base_version:
                ...
                raise Exception(
                    "Developing version can not go back in time: %s < %s"
                    % (working, tag)
                )
    ...
    if (
        tracking_branch in ["origin/develop", "origin/master"]
        or tracking_branch.startswith("origin/hotfix/")
        or pieces["distance"] == 0
    ):
        return rendered
