...
def handle_bar(context, bar_dict):
    ...
    low_price_for_atr = history_bars(
        context.s, context.open_observe_time + 1, "1d", "low"
    )
    low_price_for_extreme = history_bars(
        context.s, context.close_observe_time + 1, "1d", "low"
    )
    close_price = history_bars(context.s, context.open_observe_time + 2, "1d", "close")
    close_price_for_atr = close_price[:-1]

    atr_array = talib.ATR(
        high_price, low_price_for_atr, close_price_for_atr, timeperiod=context.atr_time
    )
    ...
    if context.trading_signal != "start":
        if context.units_hold != 0:
            context.max_add += (
                0.5 * get_atr_and_unit(atr_array, atr_array.size, portfolio_value)[0]
            )
    else:
    ...
    if cur_position > 0 and bar_dict[context.s].last < get_stop_price(
        context.first_open_price, context.units_hold, atr
    ):
        ...
        if cur_position > 0 and bar_dict[context.s].last < minn:
            ...
            if (
                bar_dict[context.s].last > context.max_add
                and context.units_hold != 0
                and context.units_hold < context.units_hold_max
                and available_cash > bar_dict[context.s].last * context.unit
            ):
    ...
    if (
        context.trading_signal != context.pre_trading_signal
        or (context.units_hold < context.units_hold_max and context.units_hold > 1)
        or context.trading_signal == "stop"
    ):
